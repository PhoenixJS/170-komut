...
if(message.author.id != "627803211348312065") return message.channel.send(bakim)
  ...
  let hakkimda = args.slice(0).join(" ");
  if (!hakkimda) return message.channel.send(" | **Lütfen Kendinden Bahset yaz.**");
  message.channel.send(" | **Hakkımda Kısmı Başarıyla ``" + hakkimda + "`` olarak ayarlandı!**");
  db.set(`phakkimda_${message.author.id}`, hakkimda);
};
exports.conf = {
  enabled: true,
  guildOnly: false,
